...
define('GESHI_LANG_ROOT', __DIR__ .'/theme/'. Config::THEME .'/');
/**
...
/** The language file directory for GeSHi
    @access private */
defined('GESHI_LANG_ROOT') or define('GESHI_LANG_ROOT', GESHI_ROOT . 'geshi' . DIRECTORY_SEPARATOR);
...
class Geshi {
     ...
     * @var string
     */
    protected $table_linenumber_style = 'width:1px;text-align:right;margin:0;padding:0 2px;vertical-align:top; background:#00007f; color:#fff';
    ...
    public function parse_code () {
        ...
        for ($key = 0; $key < $num_parts; ++$key) {
            ...
            if (isset($script_key) &&
                ...
                $geshi =  new GeSHi($part, $this->language_data['HIGHLIGHT_STRICT_BLOCK'][$script_key]);
                $geshi->enable_strict_mode(true);
                $geshi->set_header_type(GESHI_HEADER_NONE);
                $result .= str_replace('<br />', '', $geshi->parse_code());
            } elseif ($this->strict_mode && !is_null($parts[$key][0])) {
